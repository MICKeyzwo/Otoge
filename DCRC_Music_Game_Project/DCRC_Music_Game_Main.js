...
window.gameStart = function (url, score) {
  ...
  function draw() {
    ...
    for (let i = 0; i < drawQue[6].length; i++) {
      ctx.strokeStyle = "#aaa";
      ctx.lineWidth = 1;
      if (400 + 350 * ((curTime - drawQue[6][i]) / 1000) > 450) {
        drawQue[6].shift();
        i--;
        continue;
      }
      if (400 + 350 * ((curTime - drawQue[6][i]) / 1000) > 50) {
        ctx.beginPath();
        ctx.moveTo(50, 400 + 350 * ((curTime - drawQue[6][i]) / 1000));
        ctx.lineTo(650, 400 + 350 * ((curTime - drawQue[6][i]) / 1000));
        ctx.stroke();
      }
    }
    ...
    for (let i = 0; i < 6; i++) {
      for (let j = 0; j < drawQue[i].length; j++) {
        if (drawQue[i][j][1] == 1 || drawQue[i][j][1] == 2) {
          if (400 + 350 * ((curTime - drawQue[i][j][0]) / 1000) > 450) {
            drawQue[i].shift();
            effectQue[i].push([2, 0]);
            j--;
            continue;
        ...
        if (drawQue[i][j][1] == 1) {
          ...
          ctx.lineWidth = 4;
          ctx.beginPath();
          ctx.moveTo(50 + 100 * i, 400 + 350 * ((curTime - drawQue[i][j][0]) / 1000));
          ctx.lineTo(150 + 100 * i, 400 + 350 * ((curTime - drawQue[i][j][0]) / 1000));
          ctx.stroke();
          ...
          if (400 + 350 * ((curTime - drawQue[i][j][0]) / 1000) > 450) {
            ...
            start = 400 + 350 * ((curTime - drawQue[i][j][0]) / 1000);
          }
          let end;
          if (j == drawQue[i].length - 1) {
            end = 50;
          } else {
            end = 400 + 350 * ((curTime - drawQue[i][j + 1][0]) / 1000);
          }
          ...
          if (holdOn[i] == 1) {
            start = 400;
            if (400 + 350 * ((curTime - drawQue[i][j][0]) / 1000) < 50) {
              end = 50;
            } else {
              end = 400 + 350 * ((curTime - drawQue[i][j][0]) / 1000);
            }
            if (end >= 400) {
              drawQue[i].shift();
              effectQue[i].push([0, 0]);
  ...
  function getVideoResource(id, cb) {
    let script = document.createElement("script");
    script.src = "https://script.google.com/macros/s/AKfycbzRar-qnE70FMUkIBnd8HmXvs3d8Y3p2clG2TaJN-lNEnvRl5o/exec";
    script.src += "?id=" + id;
    let name = "_cb";
    while (window[name]) name = "_" + name;
    script.src += "&callback=" + name;
    window[name] = function (json) {
      if (json.urlData && json.urlData["hd720:video/mp4"]) {
        cb(json.urlData["hd720:video/mp4"]);
      }
      delete window[name];
    }
